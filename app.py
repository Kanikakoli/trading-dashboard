...
# ------------------------------------------------------------------
st.set_page_config(
    page_title="PRO TERMINAL v23.3 (MANUAL OVERRIDE)",
    page_icon="⚡",
    layout="wide",
...
# Initialize Session State for Manual Custom Trades if not exists
if "custom_trades_store" not in st.session_state:
    st.session_state.custom_trades_store = [
        {"sym": "NIFTY 24300 CE", "index": "NIFTY 50", "expiry": "06 AUG 2026", "ltp": 101.70, "rec": "STRONG BUY", "acc": "94.2% Accuracy", "entry": 98.0, "sl": 85.0, "target": 135.0, "budget": "₹15,000", "trail": "Active"},
        {"sym": "BANKNIFTY 57100 CE", "index": "BANK NIFTY", "expiry": "05 AUG 2026", "ltp": 340.20, "rec": "BUY", "acc": "91.5% Accuracy", "entry": 325.0, "sl": 295.0, "target": 410.0, "budget": "₹25,000", "trail": "Active"}
    ]
...
# ------------------------------------------------------------------
# 4. LIVE MARKET & GLOBAL DATA ENGINE (WITH FALLBACKS)
# ------------------------------------------------------------------
tickers = {
    "NIFTY 50": "^NSEI",
    "BANK NIFTY": "^NSEBANK",
...
with col_h1:
    st.markdown(f"<h3 style='margin:0; font-size:12px; font-weight:900; color:#0F172A;'>⚡ PRO TERMINAL (MANUAL OVERRIDE ENABLED)</h3><div style='font-size:8px; color:#64748B;'>Time: {current_time}</div>", unsafe_allow_html=True)
with col_h2:
...
# ------------------------------------------------------------------
# 5. MASTER NAVIGATION TABS (INCLUDING MANUAL OVERRIDE PAGE)
# ------------------------------------------------------------------
main_pages = st.tabs([
...
# --- PAGE 1: MANUAL PRICE & EXPIRY OVERRIDER (SOLUTION PAGE) ---
with main_pages[1]:
...
with main_pages[7]:
    ...
    if os.path.isfile("trade_performance.csv") and st.button("🗑️ Clear History"):
